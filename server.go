...
type Handler interface {
	OnConnect(c *Conn)               // OnConnect 当TCP长连接建立成功时回调
	OnMessage(c *Conn, bytes []byte) // OnMessage 当客户端有数据写入时回调
...
func NewServer(address string, handler Handler, opts ...Option) (*Server, error) {
	...
	readBufferPool := &sync.Pool{
		New: func() interface{} {
			return make([]byte, options.readBufferLen)
		},
	}
...
// startIOProducer 启动生产者
func (s *Server) startIOProducer() {
	log.Info("egoll start io producer")
	for {
		select {
		case <-s.stop:
